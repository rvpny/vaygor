...
$venuePrice = (float) ($venue['price'] ?? 0);
...
function vaygor_slot_grid(string $date, int $fid, mysqli $conn): array
{
    $out = [
        'ok' => false,
        'message' => '',
    ...
    $busy = [];
    try {
        $stmt = $conn->prepare("SELECT start_time, end_time FROM bookings WHERE id_field=? AND booking_date=? AND status IN ('pending','confirmed')");
...
if (isset($_POST['book'])) {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("SELECT id FROM bookings WHERE id_field=? AND booking_date=? AND status IN ('pending','confirmed') AND NOT (end_time <= ? OR start_time >= ?) LIMIT 1");
        $st = $oldStart . ':00';
        $et = $oldEnd . ':00';
    ...
    if (empty($errors)) {
        ...
        if ($stmt->execute()) {
            ...
            $stmt->execute();
            $stmt->close();
            $success = ['code' => $code, 'date' => $oldDate, 'start' => $oldStart, 'end' => $oldEnd, 'total' => $total];
        } else {
...
$grid = vaygor_slot_grid($oldDate, $fid, $conn);
?>
<div class="pt-20">
  <div class="mx-auto max-w-3xl px-6 sm:px-8 lg:px-12 py-8 pb-24 lg:pb-12">
    ...
    <?php if ($success): ?>
      <div class="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-6">
        ...
        <p class="mt-1 text-sm text-emerald-700">Kode: <span class="font-mono font-bold"><?php echo htmlspecialchars($success['code'], ENT_QUOTES, 'UTF-8'); ?></span> • <?php echo htmlspecialchars($success['date'], ENT_QUOTES, 'UTF-8'); ?> <?php echo htmlspecialchars($success['start'], ENT_QUOTES, 'UTF-8'); ?>-<?php echo htmlspecialchars($success['end'], ENT_QUOTES, 'UTF-8'); ?> • <?php echo htmlspecialchars(fmtRp($success['total']), ENT_QUOTES, 'UTF-8'); ?></p>
        <p class="mt-1 text-xs text-emerald-700">Selesaikan pembayaran di halaman pesanan agar slot dikonfirmasi.</p>
